...
set rsi=conn.execute("select * from class")
	x=0
...
<%
	...
	set dhrs=server.createobject("adodb.recordset")
	dhsql="select * from class where id="&cid&""
...
%>
	<head>
		...
		<title><%=web_name%> | <%=classname%></title>
...
<meta name="description" content="<%=web_description%>" />
	...
	<% 
set rs=server.createobject("adodb.recordset")
sql="select * from riji where classid="&cid&" order by w_id desc"
rs.open(sql),conn,1,1
