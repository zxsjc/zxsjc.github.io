<%
'=========================================================
 'Îc½È?: AnUpLoad(?åß?™“??ëJÎË?ëM?ÑÚ)
 'ë??? Anlige
 '???: ?åß?ASP™“??ëJÎË?ëM?ÑÚV11.03.25
 '“Ò??¢‡?òå? 2008-4-12
 'ì‡¸ñ§P™“½»?: 2011-03025
 'ã–²µ¡÷: http://dev.mo.cn
...
Class AnUpLoad
	...
	Private Sub Class_Initialize()
		...
		vVersion = "?åß?ASP™“??ëJÎË?ëM?ÑÚV10.10.22"
		vMaxSize = -1
		vSingleSize = -1
		vErr = -1
		vExe = ""
	...
	Public Sub GetData()
		...
		'ã–?Ñ¹3?½»µö???âˆ??ŽÐ?Ó¼?³ÁIS5.0ã–ÉÚ?ã´??ã–´¡?‘’Åú?‘’ÕÒ??¡H?‘’Åú?‘Ä??ëJ‰V?ëMØ‡?ÉM??ã–??äÕ´É”ËÏþ?ãÍ???Ä¯?????ÖÕ??³ÁIS5ëJØÀ?‘Ä??òåÆöòù?¶ç??³ôÚ¿???ã–?Ñ¹3?Òà?¯ŒÈ¯Ñé
		'If Not ef Then
        ...
        Do
			...
			If InStr(str, "filename=""") > 0 Then
				...
				If Trim(FileName) <> "" Then
					...
					If vSingleSize > 0 And (valueend - formend - 6) > vSingleSize Then '?¶·þ€ã–´¡??¨û¿û??À±‘’Åú?
						vErr = 5
						tempdata = empty
						Exit Sub
					End If
					If vMaxSize > 0 And vTotalSize > vMaxSize Then '?¶·þ€ã–´¡??à¦?ö¤½ÈÖ®“Ø?
						vErr = 1
						tempdata = empty
						Exit Sub
					End If
	...
	Private Function GetErr(ByVal Num)
		Select Case Num
			Case 0
				GetErr = "?à¦?‘’??“µ??!"
			Case 1
				GetErr = "ã–´¡??à¦?îO?ÂÕ" & GetSize(vMaxSize) & "?¡H?!?º½ÍìÎ~Ì×axSize¾ï??°â??·õ??¡H?!"
			Case 2
				GetErr = "òåÜÀÍìÎ~Í©?ëM?¡±?¥ûnctype¾ï??ÕÒÓ¼multipart/form-data?????ö¬êÊÒñmethod¾ï??ÕÒÓ¼Post,ã–´¡?™“??!"
			Case 3
				GetErr = "?¼±?Øá??”Ë×¾???" & vErrExe & ")??À±!?ÜÀ?ã–´¡?”Ë×¾??—ùÓ¼" & Replace(vExe, "|", ",") & "‘Ä??ëJ?
			Case 4
				GetErr = "ÞþæÌ?íj?Îf´g?ã–??ö¬ö¤òù?º¯???ame¾ï??Êã???À±??"
			Case 5
				GetErr = "?¨û¿û??À±‘’Åú?îO??" & GetSize(vSingleSize) & "‘Ä??ëM????"
		End Select
	End Function
	...
	'==============================
	'çö?¯‹??ëM?ÑÚ????³æÓ¼multipart/form-data
	'==============================
	...
	'==============================
	'újè¦?ã–´¡?äLµä?òå???§Ì?ô³??ã´??ã–?1?øa?ŽÐ??ã–Ü¿??¼±?òå?¡±?©[¡ï‘Ä??ã–ÄàictionaryÞþ¹ÑÇé
	'==============================
	...
	'==============================
	'újè¦?ã–´¡?‘Ä??ëJàÆÑÚ,??§Ì?ô³??ã´??ã–?1?øa?ŽÐ??ã–Ü¿??¼±?òå??ëM??ëJàÆÑÚ‘Ä??ã–ÄàictionaryÞþ¹ÑÇé
	'==============================
